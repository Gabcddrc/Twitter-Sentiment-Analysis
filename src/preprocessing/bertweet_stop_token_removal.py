## BERTweet with removing non-discriminatory stop token determined by analysing
## the given dataset and filtering for frequent token with almost equal
## probability of occuring in positive or negative tweet.
...
class StopTokenRemover(Normalizer):
    ...
    def __init__(self) -> None:
        super(StopTokenRemover, self).__init__()
        stop_token_candidates = pd.read_csv("stop_token_candidates.csv")
        self.stop_token = set(stop_token_candidates["token"])

    def normalize(self, tweet: str) -> str:
        tokens = tweet.split()
        filtered_tokens = [token for token in tokens if token not in self.stop_token]

        self.metric += len(tokens) - len(filtered_tokens)

        return " ".join(filtered_tokens)


stop_token_remover = StopTokenRemover()
...
def main():
    ...
    experiment.set_name("BERTweet Stop Token Removal")
    ...
    print("load data")
    train["tweet"] = train["tweet"].apply(stop_token_remover.normalize)
    valid["tweet"] = valid["tweet"].apply(stop_token_remover.normalize)
    ...
    metrics = {"train_score": float(train_accuracy), "valid_score": float(val_accuracy)}
    ...
    test["tweet"] = test["tweet"].apply(stop_token_remover.normalize)
