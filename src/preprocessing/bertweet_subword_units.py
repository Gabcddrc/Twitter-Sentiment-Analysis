## BERTweet with subword unit preprocessing for entire dataset.
...
def main():
    ...
    experiment.set_name("BERTweet Subwords")
    ...
    path_to_dataset = os.path.join("dataset-subwords")
    train, valid = load_train_valid_data(path_to_dataset)
    test = load_test_data(path_to_dataset)
    print("load data")

    # Pandas unnecessarily introduced quotation marks at start and end when
    # the tweet itself contained a comma (,), so remove those now to be in line
    # with original dataset.
    train["tweet"] = train["tweet"].apply(lambda tweet: tweet.strip('"'))
    valid["tweet"] = valid["tweet"].apply(lambda tweet: tweet.strip('"'))
    test["tweet"] = test["tweet"].apply(lambda tweet: tweet.strip('"'))
