...
class Model(nn.Module):
	def __init__(self):
		...
		self.dense0 = nn.Linear(self.transformer.config.hidden_size, 256)
	...
	def forward(self, input_ids, attention_mask):
		 ...
		 d0 = self.dropout0(transformer_output[1])
		 d0 = self.dense0(d0)
		 d0 = self.relu(d0)
		 d1 = self.dropout1(d0)
		 d1 = self.dense1(d1)
...
def main():
	...
	experiment.set_name('bertweet_multiOut_2_full')
	...
	train = train.append(valid,ignore_index=True, sort=False)
	...
	with experiment.train():
		...
		for epoch in range(1, EPOCHS+1):
			...
			torch.save(model.state_dict(), 'bertweet_multiOut_2_full.model')

			experiment.log_metric("train_epoch_loss", loss_train, epoch=epoch)   


	# load Best model
	model.load_state_dict(torch.load("bertweet_multiOut_2_full.model"))
	...
	experiment.log_table(
		filename = "bertweet2_multiout2_full_prediction.csv",
		tabular_data = predictions
	)
    predictions.to_csv("bertweet2_multiout2_full_prediction.csv")
