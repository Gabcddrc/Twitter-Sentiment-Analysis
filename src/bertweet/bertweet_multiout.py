...
def main():
	...
	experiment.set_name('bertweet_multiOut_2')
	...
	with experiment.train():
		...
		for epoch in range(1, EPOCHS+1):
			...
			if(accuracy > best_accuracy):
				...
				torch.save(model.state_dict(), 'bertweet2_3.model')
			else:
				# unfreeze pre-trained weights
				for param in model.transformer.parameters():
					param.requires_grad = True
	...
	# load Best model
	model.load_state_dict(torch.load("bertweet2_3.model"))
	...
	experiment.log_table(
		filename = "bertweet2_3_prediction.csv",
		tabular_data = predictions
	)
    predictions.to_csv("bertweet2_3_prediction.csv")
...
# bsub -n 6 -W 48:00 -o log_test_bertweet -R "rusage[mem=2500, ngpus_excl_p=1]" python bertweet.py
